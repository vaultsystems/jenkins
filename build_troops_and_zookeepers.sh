...
nova boot --flavor m1.tiny --block-device source=image,id=$ZK_IMAGE,dest=volume,size=3,shutdown=remove,bootindex=0 --nic net-id=$NET --key-name monkey_pm zookeeper$i;
sleep $ZK_INTERVAL;
done
